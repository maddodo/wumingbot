...
class Telegram
{
    ...
    public function finderror($msg){
        global $chat_id;
        if(DEBUG) $content = ['chat_id' => ADMIN_ID, 'text' => $msg];
        else $content = ['chat_id' => $chat_id, 'text' => '抱歉，机器人遇到了一些问题，请联系管理员处理！'];
     ...
     * \return the JSON Telegram's reply.
     */
    public function sendMessage(array $content, $split = false)
    {
        if($split){
            $extras = [
                'split'    => 4096,
     ...
     * @param {*} $chat_id
     * @param {*} $action typing upload_photo record_video upload_video record_voice upload_voice upload_document choose_sticker find_location record_video_note upload_video_note
    ...
    /**
     * @description: 获取群组里的群员
     * @param {string} $chat_id
     * @param {string} $user_id
     * @return {array} a ChatMember object on success.
     */
